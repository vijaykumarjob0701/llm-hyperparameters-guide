...
A beginner-friendly guide to the knobs you turn **after** you already have a base model and a dataset. The long version lives in this README (it *is* the blog post). A shorter narrative is in [BLOG.md](BLOG.md). The looping GIFs regenerate from [`scripts/generate_gifs.py`](scripts/generate_gifs.py).
...
![Learning rate: high steps overshoot the loss bowl, a typical step walks downhill, a tiny step crawls](docs/gifs/learning_rate.gif)
...
![Batch size: one-example gradients scatter; the mini-batch mean settles toward the true gradient](docs/gifs/batch_size.gif)
...
![Epochs: train loss keeps falling while held-out loss drops, then rises into overfitting](docs/gifs/epochs.gif)
...
![Warmup ratio: a schedule that starts at zero versus one that slams into the peak LR](docs/gifs/warmup_ratio.gif)
...
![Weight decay: unconstrained weights wander outward; a small decay keeps them in a ball](docs/gifs/weight_decay.gif)
...
![Optimizer: SGD zigzags in a skinny valley while AdamW takes a calmer path to the minimum](docs/gifs/optimizer.gif)
...
![LoRA rank: a needed update matrix versus low-rank reconstructions at r = 1, 4, 16](docs/gifs/lora_rank.gif)
...
![LoRA alpha: the same low-rank update added to Wx at increasing volume](docs/gifs/lora_alpha.gif)
...
![LoRA dropout: adapter units randomly silenced in training, all on at inference](docs/gifs/lora_dropout.gif)
...
![Target modules: attention-only versus attention+MLP adapters on one decoder block](docs/gifs/target_modules.gif)
...
![Max sequence length: tokens past the red cutoff are truncated, often the assistant answer](docs/gifs/max_seq_length.gif)
...
![Temperature: a peaked next-token distribution at low T versus a flatter one at high T](docs/gifs/temperature.gif)
...
![Top-p: the highlighted nucleus is the smallest set of tokens whose probability mass reaches p](docs/gifs/top_p.gif)
...
![Top-k: only the k tallest bars stay in the menu; the rest are zeroed](docs/gifs/top_k.gif)
...
![max_new_tokens: generation walks token by token into a hard cutoff](docs/gifs/max_tokens.gif)
...
![Stop sequences: generation hits a marker and unused token budget is left on the table](docs/gifs/stop_sequences.gif)
...
python scripts/generate_gifs.py --only temperature top_p
python scripts/generate_gifs.py --list
```

Outputs land in [`docs/gifs/`](docs/gifs/). No Hugging Face token and no 7B download are required.
