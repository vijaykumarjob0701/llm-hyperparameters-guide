...
The long, linkable version (every knob, a GIF, ranges, mistakes, code) is the [README](README.md). This page is the narrative you can read in one sitting.
...
Pictures for every knob, the when-to-tune table, and copy-paste `TrainingArguments` / `LoraConfig` / `generate` blocks live in the [README](README.md). Rebuild the GIFs with `python scripts/generate_gifs.py`.
